...
import AdminOfferedHomeRow from "../../components/AdminOfferedHomeRow.jsx";

const Offeredhomes_1 = observer(() => {
  useEffect(() => {
    const handleResize = () => {
      const bodyWidth = document.body.clientWidth;
      const scale = bodyWidth / 1440;
      const element = document.getElementById("t18_1330_381");
      if (element) {
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    >
                      {dl.changes.offeredHomeList.map((item, index) => {
                        return (
                          <div>
                            <AdminOfferedHomeRow
                              {...item}
                              {...dl.changes.offeredHomeList}
                              onActivate={(val) => {
                                var id = dl.changes.offeredHomeList[index]._id;
                                var body = {
                                  id: id,
                                  status: "active",
                                };

                                dl.functions.updateOfferedHome(body);
                              }}
                              onDisable={(val) => {
                                var id = dl.changes.offeredHomeList[index]._id;
                                var body = {
                              ...
                              onChangeMenu={(val) => {
                                if (val == "Edit Promotion") {
                                  dl.changes.selectedPromotion =
                                    dl.changes.offeredHomeList[index];
                                  dl.changes.promotionTitle =
                                    dl.changes.offeredHomeList[index].title;
                                  dl.changes.promotiondata =
                                    dl.changes.offeredHomeList[index].promotion;
                                  if (
                                ...
                                if (val == "View More") {
                                  var id =
                                    dl.changes.offeredHomeList[index]._id;
                                  window.open(
                                    `https://mm-dev-64390b599811daae82a555cf.app.neutrodev.com/offeredhomedetails?homeId=${id}`,
                                    "_blank",
                                  );
                                }
                              }}
                  ...
                  >
                    <MMPagination
                      ...
                      onChange={(val) => {
                        ...
                        dl.functions.getOfferedHome("6", val);
                      }}
                    />
                  </div>
                </div>
